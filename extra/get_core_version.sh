...
# GITHUB_HEAD_REF points to the branch or tag ref that triggered the workflow run
# (e.g., "refs/heads/main" or "refs/tags/v1.2.3"). Without this, git describe
# would use HEAD, which is a temporary detached commit in GitHub Actions.
# If GITHUB_HEAD_REF is not set, the command falls back to using HEAD.

VERSION=$(git describe --tags --exact-match ${GITHUB_HEAD_REF} 2>/dev/null)
if [ -z "$VERSION" ]; then
	VERSION=$(git describe --tags --dirty ${GITHUB_HEAD_REF} 2>/dev/null |
