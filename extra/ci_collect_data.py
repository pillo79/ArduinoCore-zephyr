...
#     - PR_LABELS: optional, JSON array of the current pull request's label names
#       (or '[]' outside of a pull request). Labels of the form 'ci-ignore:<pattern>'
#       add <pattern> as an extra exception/warning filter (same format as
#       known_example_issues.txt below), applied to every board, letting maintainers
#       silence failures caused by outside factors without a code change. This works
#       for forked PRs too, unlike a repository variable.
#
...
def parse_known_issues(lines, sketch_anchor=r"^(ArduinoCore-zephyr/)?"):
    """
    ...
    sketch_anchor is prepended to every sketch-path regular expression before
    compiling it. The default anchors at the start of the (repo-relative)
    sketch path, as used for known_example_issues.txt entries; pass ".*"
    instead to match the pattern anywhere in the path (e.g. for PR labels,
    where only the example name is given, not its full path).
    ...
    for line in lines:
        ...
        if not line:
            ...
            sketch_pattern = re.compile(f"{sketch_anchor}{sketch_pattern}")
            warning_pattern = re.compile(warning_pattern)
            warning_filters.append(( sketch_pattern, warning_pattern ))
        else:
            # this is an exception
            exceptions.append(re.compile(f"{sketch_anchor}{line}"))
...
# Extra exceptions/warning filters applying to every board, sourced from any
# 'ci-ignore:<pattern>' labels on the current pull request (see PR_LABELS above).
CI_IGNORE_LABEL_PREFIX = 'ci-ignore:'
PR_LABELS = json.loads(os.environ.get('PR_LABELS', '[]'))
IGNORE_PATTERNS = [
    label[len(CI_IGNORE_LABEL_PREFIX):] for label in PR_LABELS
    if label.startswith(CI_IGNORE_LABEL_PREFIX)
]
GLOBAL_EXCEPTIONS, GLOBAL_WARNING_FILTERS = parse_known_issues(IGNORE_PATTERNS, sketch_anchor=".*")
...
for board_data in ALL_BOARD_DATA.values():
    ...
    exceptions = list(GLOBAL_EXCEPTIONS)           # [ compiled_sketch_pattern, ...]
    warning_filters = list(GLOBAL_WARNING_FILTERS) # [ (sketch_pattern, regex), ...]
    if os.path.exists(f"variants/{variant}/known_example_issues.txt"):
        with open(f"variants/{variant}/known_example_issues.txt", 'r') as f:
            variant_exceptions, variant_warning_filters = parse_known_issues(f)
            exceptions += variant_exceptions
            warning_filters += variant_warning_filters
