...
/* If the CGC registers are only accessible in Secure mode,
 * than there is no reason for Non Secure applications to access
...
#define BSP_CFG_ROM_REG_OFS1_SEL \
		...
		((0U << 0U)) | \
		((0U << 2U)) | \
		((BSP_CFG_CLOCKS_SECURE == 0) ? 0x700U : 0U))
#else
